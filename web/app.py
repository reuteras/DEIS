...
def resolve_and_verify_target_file(symlink_path: Path) -> Path:
    """Resolve symlink and verify target is within allowed directory.

    Follows symlink and verifies the resolved path is safe.
    Returns the resolved Path object verified to exist as a symlink target.
    """
...
def convert_to_pdf(file_path: Path) -> bytes:
...
def convert_html_to_pdf(file_path: Path) -> bytes:
    """Send the file to Gotenberg for conversion to PDF."""
...
async def get_file(sha256: str):
    ...
    target_file = resolve_and_verify_target_file(symlink_path)
    ...
    if mime_type in SEND_AS_IS:
        try:
            if mime_type in NO_EXTENSION:
                return FileResponse(str(target_file), media_type=mime_type)
            return FileResponse(str(target_file), media_type=mime_type, filename=validated_filename)
        except requests.RequestException as e:
            raise HTTPException(status_code=500, detail=f"Conversion Error: {e}") from e
